...
let spam message args =
  ...
  List.range 0 count
  |> List.iter ~f:(fun i ->
         Lwt.async (fun () -> Message.reply message (string_of_int i)) )
  |> Lwt.return
...
let list message args =
  ...
  >|= function
  | Ok msg -> print_endline msg.content | Error err -> print_endline err
...
let shutdown (message : Message.t) _args =
  if message.author.id = `User_id 242675474927583232 then
    client >>= Client.shutdown_all ~restart:false >|= fun _ -> exit 0
  ...
  >>= (function
        ...
        | Error e ->
            Message.reply message
              (Printf.sprintf "Failed to create guild. Error: %s" e) )
  >|= ignore
...
let delete_guilds message _args =
  ...
  let guilds =
    Cache.GuildMap.filter (fun _ (g : Guild.t) -> g.owner_id = uid) cache.guilds
  in
  let res = ref "" in
  let all =
    Cache.GuildMap.(
      map
        ...
        guilds
      |> to_seq )
    |> Seq.map snd |> Stdlib.List.of_seq
  in
  Lwt.join all >>= (fun _ -> Message.reply message !res) >|= ignore
...
let role_test (message : Message.t) args =
  ...
  let create_role name guild_id =
    ...
    >|= function Ok role -> role | Error e -> raise (Failure e)
  in
  let delete_role role =
    Role.delete role >|= function Ok () -> () | Error e -> raise (Failure e)
  in
  let add_role member role =
    Member.add_role ~role member
    >|= function Ok () -> role | Error e -> raise (Failure e)
  in
  ...
  in
  let get_member id =
    match Cache.GuildMap.find_opt id cache.guilds with
    | Some guild -> (
      match
...
let check_permissions (message : Message.t) _args =
  ...
  let permissions =
    ...
    | Some g, Some m -> (
      match Cache.guild g cache with
      | Some g ->
  ...
  let allow, deny =
    ...
    | Some m -> (
      match Cache.text_channel message.channel_id cache with
      | Some c ->
