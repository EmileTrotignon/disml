...
module Base = struct
  ...
  let process_response path ((resp : Response.t), body) =
    let limit =
      ...
      | None ->
          Rl.default
    in
    Lwt_mvar.put (Rl.find path !rl) limit
    >>= fun () ->
    ...
    | code ->
        ...
        >>= fun () ->
        Lwt_result.fail
        @@ Printf.sprintf "Unsuccessful response received: %d - %s" code body
  ...
  let request ?(body = `Null) ?(query = []) m (endpoint : Endpoints.t) =
    ...
    if limit.remaining > 0 then process ()
      ...
      Logs_lwt.info (fun m ->
          m "Rate-limiting [Route: %s] [Duration: %f s]" endpoint.route time )
      >>= fun () -> Lwt_unix.sleep time >>= process
end

let r_map f = function Ok v -> Ok (f v) | Error e -> Error e
...
let get_channel channel_id =
  ...
  >|= r_map (fun c -> Channel_t.(channel_wrapper_of_yojson_exn c |> wrap))
...
let get_reactions channel_id message_id emoji =
  ...
  >|= r_map (fun l ->
          Yojson.Safe.Util.to_list l |> List.map User_t.of_yojson_exn )
...
let create_guild body =
  ...
  >|= r_map (fun g -> Guild_t.(pre_of_yojson_exn g |> wrap))
...
let get_roles guild_id =
  ...
  >|= r_map (fun l ->
          Yojson.Safe.Util.to_list l
          |> List.map (fun r -> Role_t.(role_of_yojson_exn r |> wrap ~guild_id)) )

let guild_role_add guild_id body =
  Base.request ~body `Post (Endpoints.guild_roles guild_id)
  >|= r_map (fun r -> Role_t.(role_of_yojson_exn r |> wrap ~guild_id))
...
let get_guilds () =
  ...
  >|= r_map (fun l ->
          Yojson.Safe.Util.to_list l
          |> List.map (fun g -> Guild_t.(pre_of_yojson_exn g |> wrap)) )
