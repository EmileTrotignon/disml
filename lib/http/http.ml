...
module Base = struct
  ...
  let process_url path = Uri.of_string (base_url ^ path)

  let process_request_body body =
    body |> Yojson.Safe.to_string |> Cohttp_async.Body.of_string
  ...
  let process_response path ((resp : Response.t), body) =
    let limit =
      match Response.headers resp |> Rl.rl_of_header with
      | Some r ->
          r
      | None ->
          Rl.default
    in
    Mvar.put (Rl.find_exn !rl path) limit
    >>= fun () ->
    ...
    | code ->
        ...
        Logs.warn (fun m ->
            m "[Unsuccessful Response] [Code: %d]\n%s\n%s" code body headers ) ;
        Deferred.Or_error.errorf "Unsuccessful response received: %d - %s" code
          body
  ...
  let request ?(body = `Null) ?(query = []) m path =
    ...
    let process () =
      ...
      ( match m with
      | `Delete ->
          Cohttp_async.Client.delete ~headers ~body uri
      | `Get ->
          Cohttp_async.Client.get ~headers uri
      | `Patch ->
          Cohttp_async.Client.patch ~headers ~body uri
      | `Post ->
          Cohttp_async.Client.post ~headers ~body uri
      | `Put ->
          Cohttp_async.Client.put ~headers ~body uri )
      >>= process_response path
    in
    if limit.remaining > 0 then process ()
    else
      let time = Time.(Span.of_int_sec limit.reset |> of_span_since_epoch) in
      Logs.debug (fun m ->
          m "Rate-limiting [Route: %s] [Duration: %d ms]" path
            Time.(diff time (Time.now ()) |> Span.to_ms |> Float.to_int) ) ;
      Clock.at time >>= process
end

let get_gateway () = Base.request `Get Endpoints.gateway

let get_gateway_bot () = Base.request `Get Endpoints.gateway_bot
...
let get_messages channel_id limit (kind, id) =
  Base.request
    ~query:[(kind, string_of_int id); ("limit", string_of_int limit)]
    `Get
    (Endpoints.channel_messages channel_id)
  >>| Result.map ~f:(fun l ->
          Yojson.Safe.Util.to_list l |> List.map ~f:Message_t.of_yojson_exn )
...
let create_message channel_id body =
  Base.request ~body `Post (Endpoints.channel_messages channel_id)
  >>| Result.map ~f:Message_t.of_yojson_exn
...
let delete_own_reaction channel_id message_id emoji =
  Base.request `Delete
    (Endpoints.channel_reaction_me channel_id message_id emoji)
  >>| Result.map ~f:ignore

let delete_reaction channel_id message_id emoji user_id =
  Base.request `Delete
    (Endpoints.channel_reaction channel_id message_id emoji user_id)
  >>| Result.map ~f:ignore

let get_reactions channel_id message_id emoji =
  Base.request `Get
    (Endpoints.channel_reactions_get channel_id message_id emoji)
  >>| Result.map ~f:(fun l ->
          Yojson.Safe.Util.to_list l |> List.map ~f:User_t.of_yojson_exn )

let delete_reactions channel_id message_id =
  Base.request `Delete
    (Endpoints.channel_reactions_delete channel_id message_id)
  >>| Result.map ~f:ignore
...
let get_emojis guild_id =
  ...
  >>| Result.map ~f:(fun l ->
          Yojson.Safe.Util.to_list l |> List.map ~f:Emoji.of_yojson_exn )
...
let create_guild body =
  ...
  >>| Result.map ~f:(fun g -> Guild_t.(pre_of_yojson_exn g |> wrap))
...
let get_guild_channels guild_id =
  ...
  >>| Result.map ~f:(fun l ->
          ...
          |> List.map ~f:(fun g ->
                 Channel_t.(channel_wrapper_of_yojson_exn g |> wrap) ) )
...
let get_member guild_id user_id =
  ...
  >>| Result.map ~f:(fun m ->
          Member_t.(member_of_yojson_exn m |> wrap ~guild_id) )
...
let get_bans guild_id =
  ...
  >>| Result.map ~f:(fun l ->
          Yojson.Safe.Util.to_list l |> List.map ~f:Ban_t.of_yojson_exn )

let get_ban guild_id user_id =
  Base.request `Get (Endpoints.guild_ban guild_id user_id)
  >>| Result.map ~f:Ban_t.of_yojson_exn
...
let get_roles guild_id =
  ...
  >>| Result.map ~f:(fun l ->
          Yojson.Safe.Util.to_list l
          |> List.map ~f:(fun r ->
                 Role_t.(role_of_yojson_exn r |> wrap ~guild_id) ) )

let guild_role_add guild_id body =
  Base.request ~body `Post (Endpoints.guild_roles guild_id)
  >>| Result.map ~f:(fun r -> Role_t.(role_of_yojson_exn r |> wrap ~guild_id))
...
let guild_prune_count guild_id days =
  Base.request
    ~query:[("days", Int.to_string days)]
    `Get
    (Endpoints.guild_prune guild_id)
...
let get_invite invite_code = Base.request `Get (Endpoints.invite invite_code)
...
let get_guilds () =
  ...
  >>| Result.map ~f:(fun l ->
          Yojson.Safe.Util.to_list l
          |> List.map ~f:(fun g -> Guild_t.(pre_of_yojson_exn g |> wrap)) )
...
let get_connections () = Base.request `Get Endpoints.me_connections

let get_user user_id =
  Base.request `Get (Endpoints.user user_id)
  >>| Result.map ~f:User_t.of_yojson_exn

let get_voice_regions () = Base.request `Get Endpoints.regions
...
let get_webhook webhook_id = Base.request `Get (Endpoints.webhook webhook_id)
