...
module Shard = struct
  ...
  type 'a t =
    {mutable state: 'a; mutable stopped: bool; mutable can_resume: bool}
  ...
  let parse ~compress (frame : [`Ok of Frame.t | `Eof]) =
    ...
    | `Ok s -> (
        ...
        match s.opcode with
        | Text ->
            `Ok (Yojson.Safe.from_string s.content)
        | Binary ->
            if compress then
              `Ok (decompress s.content |> Yojson.Safe.from_string)
  ...
  let push_frame ?payload ~ev shard =
    ...
    Pipe.write_if_open write @@ Frame.create ~content () >>| fun () -> shard
  ...
  let heartbeat shard =
    ...
    | i ->
        Logs.debug (fun m ->
            m "Heartbeating - Shard: [%d, %d] - Seq: %d" (fst shard.id)
              (snd shard.id) shard.seq ) ;
  ...
  let set_status ?(status = "online") ?(kind = 0) ?name ?since ?url shard =
    ...
    let payload =
      `Assoc
        ...
        ; ("game", game) ]
    in
    Ivar.read shard.ready
    >>= fun _ -> push_frame ~payload ~ev:STATUS_UPDATE shard
  ...
  let initialize ?data shard =
    ...
    let _ =
      match data with
      | Some data ->
          Ivar.fill_if_empty shard.hb_interval
            (Time.Span.create
               ~ms:J.(member "heartbeat_interval" data |> to_int)
               () )
      | None ->
    ...
    | None ->
        Mvar.take identify_lock
        >>= fun () ->
        Logs.debug (fun m ->
            m "Identifying shard [%d, %d]" (fst shard.id) (snd shard.id) ) ;
        let payload =
          `Assoc
            ...
            ; ("shard", `List shards) ]
        in
        push_frame ~payload ~ev:IDENTIFY shard >>| fun s -> s
    | Some s ->
        let payload =
          `Assoc
            [ ("token", `String !Client_options.token)
  ...
  let handle_frame ~f shard =
    ...
    | INVALID_SESSION ->
        Logs.err (fun m ->
            m "Invalid Session on Shard [%d, %d]: %s" (fst shard.id)
              (snd shard.id)
  ...
  let rec make_client ~initialized ~extra_headers ~app_to_ws ~ws_to_app
      ~net_to_ws ~ws_to_net ?(ms = 500) uri =
    client ~initialized ~extra_headers ~app_to_ws ~ws_to_app ~net_to_ws
      ~ws_to_net uri
    >>> fun res ->
    match res with
    | Ok () ->
    ...
    | Error _ ->
        ...
        Clock.after backoff
        >>> fun () ->
        make_client ~initialized ~extra_headers ~app_to_ws ~ws_to_app ~net_to_ws
          ~ws_to_net
          ~ms:(min 60_000 (ms * 2))
          uri
  ...
  let shutdown ?(clean = false) ?(restart = true) t =
    ...
    t.stopped <- true ;
    Logs.debug (fun m ->
        m "Performing shutdown. Shard [%d, %d]" (fst t.state.id)
          (snd t.state.id) ) ;
...
let start ?count ?compress ?large_threshold () =
  ...
  >>= fun data ->
  let data = match data with Ok d -> d | Error e -> Error.raise e in
  ...
  let rec ev_loop (t : Shard.shard Shard.t) =
    let step (t : Shard.shard Shard.t) =
      ...
      >>| fun () -> t
    in
    if t.stopped then return () else step t >>= ev_loop
  in
    ...
    | id, total ->
        let wrap ?(reuse : Shard.shard Shard.t option) state =
          ...
          | Some t ->
              ...
              return t
          | None ->
              return Shard.{state; stopped= false; can_resume= true}
        in
        let create () =
        ...
        let rec bind (t : Shard.shard Shard.t) =
          let _ =
            Ivar.read t.state.hb_interval
            >>> fun hb ->
            Clock.every' ~stop:(Ivar.read t.state.hb_stopper)
              ~continue_on_error:true hb (fun () ->
                Shard.heartbeat t.state >>| ignore )
          in
          (ev_loop t >>> fun () -> Logs.debug (fun m -> m "Event loop stopped.")) ;
          ( Pipe.closed (fst t.state.pipe)
          >>> fun () ->
...
let set_status ?status ?kind ?name ?since ?url sharder =
  Deferred.all
  @@ List.map
       ~f:(fun t -> Shard.set_status ?status ?kind ?name ?since ?url t.state)
       sharder.shards

let request_guild_members ?query ?limit ~guild sharder =
  Deferred.all
  @@ List.map
       ~f:(fun t -> Shard.request_guild_members ~guild ?query ?limit t.state)
       sharder.shards

let shutdown_all ?restart sharder =
  Deferred.all
  @@ List.map ~f:(fun t -> Shard.shutdown ~clean:true ?restart t) sharder.shards
