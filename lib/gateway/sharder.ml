...
exception Inflate_error of [`Msg of string]
...
let decompress str =
  ...
  in
  (let* () = Zl.Higher.uncompress ~allocate ~refill ~flush i o in
   Buffer.contents r )
  |> function Ok v -> v | Error e -> raise (Inflate_error e)

(*
let decompress src =
  let in_buf = Bytes.create 0xFFFF in
  let out_buf = Bytes.create 0xFFFF in
  let window = Zl.Higher.reset window in
  let pos = ref 0 in
  let src_len = String.length src in
  let res = Buffer.create src_len in
  Zl.Higher.uncompress in_buf out_buf
    (fun dst ->
      ...
      len )
    (fun obuf len ->
      Buffer.add_subbytes res obuf 0 len ;
      0xFFFF )
...
module Shard = struct
  ...
  let parse ~compress (frame : Frame.t) =
    ...
    | Text ->
        `Ok (Yojson.Safe.from_string frame.content)
    | Binary ->
        if compress then
          `Ok (decompress frame.content |> Yojson.Safe.from_string)
        else `Error "Failed to decompress"
    | Close ->
        `Close (String.sub frame.content 0 2)
    | op ->
  ...
  let push_frame ?payload ~ev shard =
    let content =
      ...
      | Some p ->
          ...
          @@ `Assoc [("op", `Int (Opcode.to_int ev)); ("d", p)]
    in
    Frame.create ~content () |> shard.send >|= fun () -> shard
  ...
  let heartbeat shard =
    ...
    | i ->
        if not shard.hb_acked then
          ...
          Logs_lwt.info (fun m ->
              m "Heartbeating - Shard: [%d, %d] - Seq: %d" shard.id
                shard.shard_count shard.seq )
          >>= fun () ->
          push_frame ~payload:(`Int i) ~ev:HEARTBEAT shard
          >|= fun shard -> {shard with hb_acked= false}
  ...
  let dispatch ~payload shard =
    ...
    let session =
      if t = "READY" then (
        ...
        let _ =
          Lwt_engine.on_timer 5.0 false (fun _ ->
              Lwt.async (fun () -> Lwt_mvar.put identify_lock ()) )
        in
        J.(member "session_id" data |> to_string_option) )
      else shard.session
    in
    Event.handle_event ~ev:t data >|= fun () -> {shard with seq; session}

  let set_status ?(status = "online") ?(kind = 0) ?name ?since ?url shard =
    let since = Option.(map (fun v -> `Int v) since |> value ~default:`Null) in
    let url = Option.(map (fun v -> `String v) url |> value ~default:`Null) in
    let game =
      match name with
      | Some name ->
          `Assoc [("name", `String name); ("type", `Int kind); ("url", url)]
      | None ->
          `Null
    in
    let payload =
      `Assoc
        ...
        ; ("game", game) ]
    in
    fst shard.ready >>= fun _ -> push_frame ~payload ~ev:STATUS_UPDATE shard
  ...
  let request_guild_members ?(query = "") ?(limit = 0) ~guild shard =
    ...
    in
    fst shard.ready
    >>= fun _ -> push_frame ~payload ~ev:REQUEST_GUILD_MEMBERS shard
  ...
  let initialize ?data shard =
    ...
    let _ =
      match data with
      | Some data ->
          Lwt.wakeup_later (snd shard.hb_interval)
            J.(member "heartbeat_interval" data |> to_int)
      | None ->
          raise Failure_to_Establish_Heartbeat
    in
    let shards = [`Int shard.id; `Int shard.shard_count] in
    match shard.session with
    | None ->
        Lwt_mvar.take identify_lock
        >>= fun () ->
        Logs_lwt.info (fun m ->
            m "Identifying shard [%d, %d]" shard.id shard.shard_count )
        >>= fun () ->
        let payload =
          `Assoc
  ...
  let handle_frame ~f shard =
    ...
    match op with
    | DISPATCH ->
        dispatch ~payload:f shard
    | HEARTBEAT ->
        heartbeat shard
    | INVALID_SESSION ->
        Logs_lwt.warn (fun m ->
            m "Invalid Session on Shard [%d, %d]: %s" shard.id shard.shard_count
              (Yojson.Safe.pretty_to_string f) )
        >>= fun () ->
    ...
    | RECONNECT ->
        initialize shard
    | HELLO ->
        initialize ~data:(J.member "d" f) shard
    | HEARTBEAT_ACK ->
        Lwt.return {shard with hb_acked= true}
    | opcode ->
        Logs_lwt.warn (fun m ->
            m "Invalid Opcode: %s" (Opcode.to_string opcode) )
        >|= fun () -> shard

  open Lwt.Syntax

  let make_client ?extra_headers uri =
    let _ = extra_headers in
    let uri = Uri.with_scheme uri (Some "https") in
    let* endp = Resolver_lwt.resolve_uri ~uri Resolver_lwt_unix.system in
    Conduit_lwt_unix.(
      let* client = endp_to_client ~ctx:(Lazy.force default_ctx) endp in
      Websocket_lwt_unix.connect ?extra_headers ~ctx:(Lazy.force default_ctx)
        client uri )

  let create ~url ~shards ?(compress = true) ?(large_threshold = 100) () =
    let uri =
      Uri.(with_query' (of_string url) [("encoding", "json"); ("v", "6")])
    in
    let extra_headers = Http.Base.process_request_headers () in
    let+ conn = make_client ~extra_headers uri in
    let recv () = read conn in
    let send = write conn in
    { compress
  ...
  let shutdown ?(clean = false) ?(restart = true) t =
    ...
    if clean then (
      ...
      Logs_lwt.info (fun m ->
          m "Performing clean %s. Shard [%d, %d]" re t.state.id
            t.state.shard_count )
      >>= fun () ->
      t.state.send (Frame.close 1000)
      >|= fun () ->
      ...
      Logs_lwt.info (fun m ->
          m "Shard closed unexpectedly, %s Shard [%d, %d]" re t.state.id
            t.state.shard_count )
      >>= fun () ->
      t.state.send (Frame.close 1001)
      >|= fun () ->
      Option.map t.state.hb_stopper ~f:(fun ev -> Lwt_engine.stop_event ev)
      |> ignore
end
...
let start ?count ?compress ?large_threshold () =
  ...
  >>= fun data ->
  let data =
    match data with Ok d -> d | Error e -> Base.Error.(of_string e |> raise)
  in
  let url = J.(member "url" data |> to_string) in
  let count =
    match count with Some c -> c | None -> J.(member "shards" data |> to_int)
  in
  ...
  let rec ev_loop (t : Shard.shard Shard.t) =
    let step (t : Shard.shard Shard.t) =
      ...
      | frame ->
          ...
          | `Ok f ->
              Shard.handle_frame ~f t.state >|= fun s -> t.state <- s
          | `Close c ->
              Logs_lwt.warn (fun m -> m "Close frame received. %s" c)
              >>= fun () -> Shard.shutdown t
          | `Error e ->
              Logs_lwt.warn (fun m -> m "Websocket soft error: %s" e)
              >>= fun () -> Lwt.return_unit
          | `Eof ->
              Logs_lwt.warn (fun m -> m "Websocket closed unexpectedly")
              >>= fun () -> Shard.shutdown t )
          >|= fun () -> t
    in
  ...
  let rec gen_shards l a =
    ...
    | id, total when id >= total ->
        Lwt.return a
    | id, total ->
        let wrap ?(reuse : Shard.shard Shard.t option) state =
          match reuse with
          | Some t ->
              t.state <- state ;
              t.stop <- Lwt.wait () ;
              Lwt.return t
          | None ->
              Lwt.return Shard.{state; stop= Lwt.wait (); can_resume= true}
        in
        let create () =
        ...
        let rec bind (t : Shard.shard Shard.t) =
          Lwt.async (fun () ->
              fst t.state.hb_interval
              >>= fun hb ->
              Logs_lwt.info (fun m -> m "Starting heartbeats")
              >|= fun () ->
              let hb = float_of_int hb /. 1000.0 in
              let ev =
                Lwt_engine.on_timer hb true (fun _ ->
                    Lwt.async (fun () ->
                        let _ = Shard.heartbeat t.state in
                        Lwt.return () ) )
              in
              t.state <- {t.state with hb_stopper= Some ev} ) ;
          Lwt.async (fun () ->
              ev_loop t
              >>= fun () -> Logs_lwt.info (fun m -> m "Event loop stopped.") ) ;
          Lwt.async (fun () ->
              fst t.stop
              >>= fun () ->
              if t.can_resume then
                create () >>= wrap ~reuse:t >>= bind >|= ignore
              else Lwt.return_unit ) ;
          Lwt.return t
        in
        create () >>= wrap >>= bind
        >>= fun t -> gen_shards (id + 1, total) (t :: a)
  in
  gen_shards shard_list [] >|= fun shards -> {shards}

let set_status ?status ?kind ?name ?since ?url sharder =
  List.map
    (fun (t : Shard.shard Shard.t) ->
...
let shutdown_all ?restart sharder =
  List.map (fun t -> Shard.shutdown ~clean:true ?restart t) sharder.shards
  |> Lwt.join
