...
let wrap s =
  match s.kind with
  | 0 ->
      `GuildText (unwrap_as_guild_text s)
  | 1 ->
      `Private (unwrap_as_dm s)
  | 2 ->
      `GuildVoice (unwrap_as_guild_voice s)
  | 3 ->
      `Group (unwrap_as_group s)
  | 4 ->
      `Category (unwrap_as_category s)
  | _ ->
      raise (Invalid_channel (channel_wrapper_to_yojson s))

let get_id (c : t) =
  match c with
  | `Group g ->
      let (`Channel_id id) = g.id in
      id
  | `Private p ->
      let (`Channel_id id) = p.id in
      id
  | `GuildText t ->
      let (`Channel_id id) = t.id in
      id
  | `GuildVoice v ->
      let (`Channel_id id) = v.id in
      id
  | `Category c ->
