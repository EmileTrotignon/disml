...
module ChannelCreate = struct
  ...
  let update_cache (cache : Cache.t) (t : t) =
    ...
    | `GuildText c ->
        let text_channels =
          C.update cache.text_channels c.id ~f:(function Some _ | None -> c)
        in
        {cache with text_channels}
    | `GuildVoice c ->
        let voice_channels =
          C.update cache.voice_channels c.id ~f:(function Some _ | None -> c)
        in
        {cache with voice_channels}
    | `Category c ->
        let categories =
          C.update cache.categories c.id ~f:(function Some _ | None -> c)
        in
        {cache with categories}
    | `Group c ->
        let groups =
          C.update cache.groups c.id ~f:(function Some _ | None -> c)
        in
        {cache with groups}
    | `Private c ->
        let private_channels =
          C.update cache.private_channels c.id ~f:(function Some _ | None -> c)
        in
...
module ChannelUpdate = struct
  ...
  let update_cache (cache : Cache.t) (t : t) =
    ...
    | `GuildText c ->
        let text_channels =
          C.update cache.text_channels c.id ~f:(function
            | Some _ ->
                c
            | None ->
                c )
    ...
    | `GuildVoice c ->
        let voice_channels =
          C.update cache.voice_channels c.id ~f:(function
            | Some _ ->
                c
            | None ->
                c )
    ...
    | `Category c ->
        let categories =
          C.update cache.categories c.id ~f:(function Some _ -> c | None -> c)
        in
        {cache with categories}
    | `Group c ->
        let groups =
          C.update cache.groups c.id ~f:(function Some _ -> c | None -> c)
        in
        {cache with groups}
    | `Private c ->
        let private_channels =
          C.update cache.private_channels c.id ~f:(function
            | Some _ ->
                c
            | None ->
                c )
  ...
  let update_cache (cache : Cache.t) t =
    ...
    if C.mem cache.private_channels t.channel_id then
      let private_channels =
        ...
        | Some c ->
            C.set cache.private_channels ~key:t.channel_id
              ~data:{c with last_pin_timestamp= t.last_pin_timestamp}
        | None ->
      ...
      let text_channels =
        match C.find cache.text_channels t.channel_id with
        | Some c ->
            C.set cache.text_channels ~key:t.channel_id
              ~data:{c with last_pin_timestamp= t.last_pin_timestamp}
        | None ->
            cache.text_channels
      in
      ...
      let groups =
        match C.find cache.groups t.channel_id with
        | Some c ->
            C.set cache.groups ~key:t.channel_id
              ~data:{c with last_pin_timestamp= t.last_pin_timestamp}
        | None ->
            cache.groups
      in
  ...
  let update_cache (cache : Cache.t) (t : t) =
    ...
    List.iter t.channels ~f:(function
      ...
      | _ ->
          () ) ;
    let text_channels =
      match C.of_alist !text with
      | `Ok m ->
          C.merge m cache.text_channels ~f:(fun ~key -> function
            | `Both (c, _) | `Left c | `Right c ->
                let _ = key in
                Some c )
      | _ ->
          cache.text_channels
...
module GuildDelete = struct
  ...
  let update_cache (cache : Cache.t) (t : t) =
    ...
    if t.unavailable then
      ...
      let unavailable_guilds =
        G.update cache.unavailable_guilds t.id ~f:(function Some _ | None -> t)
      in
      ...
      | Some g ->
          ...
          List.iter g.channels ~f:(function
            | `GuildText c ->
                text_channels := C.remove cache.text_channels c.id
            | `GuildVoice c ->
                voice_channels := C.remove cache.voice_channels c.id
            | `Category c ->
                categories := C.remove cache.categories c.id
            | _ ->
                () ) ;
...
module GuildMembersChunk = struct
  ...
  let update_cache (cache : Cache.t) t =
    ...
    | Some g ->
        ...
        let users = List.map t.members ~f:(fun m -> (m.user.id, m.user)) in
        let members =
          List.filter_map t.members ~f:(fun m ->
              if
                List.exists g.members ~f:(fun m' ->
                    User_id.get_id m'.user.id <> User_id.get_id m.user.id )
    ...
    if Guild_id.Map.mem cache.guilds t.guild_id then
      let guilds =
        ...
        | Some g ->
            let (`Guild_id guild_id) = t.guild_id in
            let roles =
              List.map g.roles ~f:(fun r ->
                  if Role_id.get_id r.id = Role_id.get_id t.role.id then
                    Role_t.wrap ~guild_id t.role
                  else r )
            in
            let data = {g with roles} in
            Guild_id.Map.set cache.guilds ~key:t.guild_id ~data
